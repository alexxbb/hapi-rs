...
main(int argc, char **argv) {
    ...
    int childCount;
    ENSURE_SUCCESS(HAPI_ComposeChildNodeList(&session, nodeId, HAPI_NODETYPE_SOP, HAPI_NODEFLAGS_SOP_CURVE, true,
                                             &childCount));
    ...
    for (int i = 0; i < childCount; ++i) {
        ...
        std::cout << "Count: " << geoInfo.partCount << std::endl;
...
static void printCurveInfo(HAPI_ObjectInfo &objInfo, HAPI_GeoInfo &geoInfo, HAPI_PartInfo &partInfo) {
    ...
    for (int curveIndex = 0; curveIndex < curveInfo.curveCount; ++curveIndex) {
        ...
        ENSURE_SUCCESS(
                HAPI_GetAttributeInfo(nullptr, geoInfo.nodeId, partInfo.id, "P", HAPI_ATTROWNER_POINT, &attrInfoP));
