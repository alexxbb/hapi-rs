...
impl Kind {
    fn description(&self) -> &str {
        ...
        match self {
            ...
            Kind::Hapi(HAPI_RESULT_DISALLOWED_NC_ASSET_WITH_C_LICENSE) => "DISALLOWED_NC_ASSET_WITH_C_LICENSE",
            Kind::Hapi(HAPI_RESULT_DISALLOWED_NC_ASSET_WITH_LC_LICENSE) => "DISALLOWED_NC_ASSET_WITH_LC_LICENSE",
            Kind::Hapi(HAPI_RESULT_DISALLOWED_LC_ASSET_WITH_C_LICENSE) => "DISALLOWED_LC_ASSET_WITH_C_LICENSE",
            Kind::Hapi(HAPI_RESULT_DISALLOWED_HENGINEINDIE_W_3PARTY_PLUGIN) => "DISALLOWED_HENGINEINDIE_W_3PARTY_PLUGIN",
...
impl HAPI_Error {
    pub fn new(kind: Kind, session: Option<*const ffi::HAPI_Session>) -> HAPI_Error {
        HAPI_Error {kind, session: session.map(|s|Cell::new(s))}
    }
}
        ...
        match self.kind {
            Kind::Hapi(_) => {
                if let Some(session) = &self.session {
                    let last_error = get_last_error(session.get())
                        .expect("Could not retrieve last error");
