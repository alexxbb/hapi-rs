...
use hapi_rs::{
    parameter::*,
    session::{new_in_process_session, SessionOptions},
};
use iced::keyboard::KeyCode::Space;
use iced::widget::image;
use iced::widget::pane_grid::Axis::Horizontal;
use iced::widget::{button, column, pick_list, row, slider, text, Column, Container, Image};
...
fn render_node(node: &HoudiniNode, input: f32) -> (Vec<u8>, u128) {
    ...
    node.session
        .render_cop_to_memory(node, &mut buffer, "C", "PNG")
        .expect("COP Render");
...
impl Sandbox for App {
    ...
    fn view(&self) -> Element<'_, Self::Message> {
        ...
        let noise = pick_list(
            ...
            self.noise,
            Message::NoiseSelected,
        );

        let parms = row![noise, text("Input"), slider]
            .spacing(10)
